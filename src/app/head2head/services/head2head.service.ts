...
export class Head2headService {
  ...
  public getByTeam(teamId: number, worldcup: boolean): Observable<H2H[]> {
    if (worldcup) {
      return this.httpClient.get<H2H[]>(this.h2hWorldCupUrl + '/getbyteam/' + teamId);
    }
    else {
  ...
  public getByTeams(team1Id: number, team2Id: number = null, worldcup: boolean): Observable<H2H> {
    if (worldcup) {
      return this.httpClient.get<H2H>(this.h2hWorldCupUrl + '/getbyteams/' + team1Id + '/' + team2Id);
    }
    else {
